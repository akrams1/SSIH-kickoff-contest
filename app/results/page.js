...
function PitchLines() {
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
      <g fill="none" stroke="#3c745a" strokeOpacity="0.08" strokeWidth="2">
        <line x1="0" y1="50%" x2="100%" y2="50%" />
        <circle cx="50%" cy="50%" r="120" />
        <circle cx="50%" cy="50%" r="4" fill="#3c745a" fillOpacity="0.12" stroke="none" />
      </g>
    </svg>
  );
}
...
export default function ResultsPage() {
  ...
  const fireConfetti = () => {
    ...
    const colors = ['#88bda4', '#4c9070', '#ffffff'];
    const frame = () => {
